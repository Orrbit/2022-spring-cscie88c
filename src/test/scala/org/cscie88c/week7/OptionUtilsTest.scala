  ...
  "OptionUtils" when {
    "calling fileCharCount" should {
      "return the correct number of characters in a valid file" in {
        OptionUtils.fileCharCount("data/dirty-retail-data-sample.csv").get should be(204)
      }
    }

    "calling charCountAsString" should {
      "return the correct string in a valid file" in {
        OptionUtils.charCountAsString("data/dirty-retail-data-sample.csv") should be("number of characters: 204")
      }
