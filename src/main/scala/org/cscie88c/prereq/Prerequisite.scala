...
object Prerequisite {
  ...
  def getAverageTransactionAmount(transactions: List[CustomerTransaction]): Double = {
  ...
  def getAverageTransactionAmountByMonthAndYear(transactions: List[CustomerTransaction]): Map[(String, String), Double] =
    transactions
      .groupBy { t: CustomerTransaction => getMonthAndYear(t.transactionDate) }
      .view.mapValues(getAverageTransactionAmount(_))
      .toMap
